...
{
    constexpr int test_pawn_board[64] = {
            0,  0,  -5, 0,  6,  0, -1, 0, // 0-7
...
TEST(LegalMoveGeneratorTest, test_legal_knight_moves)
{
    ...
    legalKnightMoves(&moves, test_knight_board, 5, 19);
    EXPECT_TRUE(stack_is_equal(std::vector{11, 15, 20, 22}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 17, 19);
    EXPECT_TRUE(stack_is_equal(std::vector{0, 2, 11, 27, 34, 32}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 21, 19);
    EXPECT_TRUE(stack_is_equal(std::vector<int>{}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 33, 19);
    EXPECT_TRUE(stack_is_equal(std::vector<int>{}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 37, 19);
    EXPECT_TRUE(stack_is_equal(std::vector{20, 22, 27, 43, 52, 54, 31, 47}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 27, 45);
    EXPECT_TRUE(stack_is_equal(std::vector<int>{}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 30, 45);
    EXPECT_TRUE(stack_is_equal(std::vector{15, 13, 20, 36, 47}, moves)) << display_stack(moves);
    empty_stack(&moves);

    legalKnightMoves(&moves, test_knight_board, 43, 45);
    EXPECT_TRUE(stack_is_equal(std::vector<int>{}, moves)) << display_stack(moves);
    empty_stack(&moves);
}
